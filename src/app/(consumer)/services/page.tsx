      ...
      <div className="mt-6 grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-5">
        {CATEGORY_TILES.map((t) => (
          <Link key={t.href} href={t.href} className="flex flex-col items-center gap-2">
            <span
              className={`relative flex h-16 w-16 items-center justify-center rounded-full text-3xl text-white shadow-sm transition duration-200 hover:-translate-y-1 hover:shadow-md ${t.color}`}
            >
              {t.emoji}
              {t.comingSoon && (
