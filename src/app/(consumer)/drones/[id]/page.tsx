...
import { cancelReservation, approveCompletion } from "@/lib/actions/droneReservation";
import { CancelReservationButton } from "@/components/CancelReservationButton";
import { DroneProgressSteps } from "@/components/DroneProgressSteps";
import { ReviewForm } from "@/components/ReviewForm";
import { NicepayPayButton } from "@/components/NicepayPayButton";
...
export default async function DroneReservationDetailPage({
  ...
  return (
    <div className="mx-auto max-w-lg px-4 py-10 sm:px-8">
      {payError && (
        <div className="mb-4 rounded-md border border-red-200 bg-red-50 px-3 py-2 text-xs text-red-700 dark:border-red-900/40 dark:bg-red-900/20 dark:text-red-400">
          {payError}
        </div>
      )}
      {paid === "1" && (
        <div className="mb-4 rounded-md border border-emerald-200 bg-emerald-50 px-3 py-2 text-xs text-emerald-700 dark:border-emerald-900/40 dark:bg-emerald-900/20 dark:text-emerald-400">
          결제가 완료되었습니다.
        </div>
      )}
          ...
          {needsAdditionalPayment && (
            <div className="mt-3">
              <NicepayPayButton
                ...
                goodsName={`드론 방제 면적 초과분(${reservation.cropType})`}
                buyerName={reservation.farmer.name}
                buyerTel={reservation.farmer.phone ?? undefined}
                buyerEmail={reservation.farmer.email}
                label={`${formatPrice(reservation.payment.additionalAmount)} 추가 결제하기`}
              />
            </div>
          )}
        </div>
      ...
      {reservation.status === "REQUESTED" && (
        <div className="mt-6">
          ...
          <NicepayPayButton
            ...
            goodsName={`드론 방제(${reservation.cropType}, ${reservation.areaPyeong}평)`}
            buyerName={reservation.farmer.name}
            buyerTel={reservation.farmer.phone ?? undefined}
            buyerEmail={reservation.farmer.email}
            label={`${formatPrice(reservation.totalPrice)} 결제하기`}
          />
        </div>
      )}
